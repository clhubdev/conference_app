...
describe('Feature: Change the date of conference', () => {
    async function expectedDatesRemainUnchanged() {
    ...
    beforeEach(async () => {
        ...
        useCase = new ChangeDates(repository, dateGenerator, bookingRepository, mailer, userRepository)
    })
    ...
    describe('Scenario: Conference does not exist', () => {
        ...
        it('should fail',async ()=> {
            ...
            await expectedDatesRemainUnchanged()
        })
    })

    describe('Scenario: Udpate conference of someone else', () => {
        const startDate = addDays(new Date(), 8)
        const endDate =  addDays(addHours(new Date(), 2), 8)
    ...
    describe('Scenario: The new start date is too close', () => {
        const startDate = new Date('2024-01-02T00:00:00.00Z')
        const endDate =  new Date('2024-01-02T02:00:00.00Z')
        ...
        it('should fail',async ()=> {
            await expect(useCase.execute(payload)).rejects.toThrow('The conference must happens at least 3 days')
            await expectedDatesRemainUnchanged()
        })
    })

    describe('Scenario: The uptaded conference is too long', () => {
        const startDate = new Date('2024-01-04T00:00:00.00Z')
        const endDate =  new Date('2024-01-04T05:00:00.00Z')
